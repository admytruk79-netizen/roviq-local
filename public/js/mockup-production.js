...
function pinUi(){
  ...
  if(tools){force(tools,'position','fixed');force(tools,'top','max(78px, calc(env(safe-area-inset-top) + 58px))');force(tools,'bottom','auto');force(tools,'left','22px');force(tools,'right','auto');force(tools,'transform','none');force(tools,'z-index','2200')}
  const nav=$('.bottom-nav');
  if(nav){force(nav,'position','fixed');force(nav,'left','0');force(nav,'right','0');force(nav,'top','auto');force(nav,'bottom','calc(10px + env(safe-area-inset-bottom))');force(nav,'transform','none');force(nav,'z-index','2200')}
  const locate=$('.roviq-locate-native');
  if(locate){force(locate,'position','fixed');force(locate,'left','16px');force(locate,'top','auto');force(locate,'bottom','calc(98px + env(safe-area-inset-bottom))');force(locate,'transform','none');force(locate,'z-index','2250')}
}
