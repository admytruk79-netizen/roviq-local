...
export async function onRequestPost({ request, env }) {
  ...
  if (possible.results?.length && !body.allow_duplicate) {
    return Response.json({ success: false, error: 'possible duplicate', duplicates: possible.results }, { status: 409 });
  }
  ...
  const result = await env.DB.prepare(
    `INSERT INTO places
      (name, category, description, lat, lng, address, photo_url, hours,
       is_drivers_pick, status, submitted_by, updated_at, verified_at,
       is_hidden, trust_level, moderation_note)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).bind(
    name, category, body.description || null, lat, lng, body.address || null,
    body.photo_url || null, body.hours || null, body.is_drivers_pick ? 1 : 0,
    status, body.submitted_by || 'ROVIQ curator', now, now,
    body.is_hidden ? 1 : 0, trustLevel, body.moderation_note || null
  ).run();
